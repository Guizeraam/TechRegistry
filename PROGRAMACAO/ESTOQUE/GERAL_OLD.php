<?php
	...
	$sql = "SELECT PRODUTO,DESC_PRODUTO, MAX(CUSTO) AS CUST, SUM(ESTOQUE) AS ESTO, SUM(VENDIDO) AS VEND, SUM(ENTREGUE) AS ENT
       ,SUM(CASE EMP WHEN 'MATRIZ' THEN ESTOQUE ELSE 01 END) AS EST_MATRIZ
       ,SUM(CASE EMP WHEN 'FILIAL' THEN ESTOQUE ELSE 0 END) AS EST_FILIAL
       ,SUM(CASE EMP WHEN 'PR' THEN ESTOQUE ELSE 0 END) AS EST_PR
FROM  (      SELECT 'MATRIZ' AS EMP, B1_COD AS PRODUTO, B1_DESC AS DESC_PRODUTO, B2_QATU AS ESTOQUE, B1_CUSTD AS CUSTO, SUM(C6_QTDVEN) AS VENDIDO, SUM(C6_QTDENT) AS ENTREGUE, SUBSTRING(B1_COD, 0, 7) AS FAMILIA
             FROM SB1010 SB1
             INNER JOIN SB2010 SB2 ON B1_COD = B2_COD AND B2_LOCAL = '01' AND SB2.D_E_L_E_T_ = ''
             INNER JOIN SC6010 SC6 ON B1_COD = C6_PRODUTO AND B2_FILIAL = C6_FILIAL AND C6_LOCAL = B2_LOCAL AND SC6.D_E_L_E_T_ = ''
             INNER JOIN SC5010 SC5 ON C6_NUM = C5_NUM AND C6_FILIAL = C5_FILIAL AND C5_NOTA NOT IN ('XXXXXX','XXXXXXXXX') AND SC5.D_E_L_E_T_ = ''
             WHERE SB1.D_E_L_E_T_=''
			 AND SUBSTRING(B1_COD, 0, 7) IN ('300302','300308')
             GROUP BY B1_COD, B1_DESC, B1_CUSTD, B2_QATU, C6_QTDVEN ,C6_QTDENT

             UNION 

             SELECT 'PR' AS EMP, B1_X_SECUN, B1_DESC, B2_QATU AS ESTOQUE, B1_CUSTD AS CUSTO, SUM(C6_QTDVEN) AS VENDIDO, SUM(C6_QTDENT) AS ENTREGUE, SUBSTRING(B1_X_SECUN, 0, 7) AS FAMILIA
             FROM SB1040 SB1
             INNER JOIN SB2010 SB2 ON B1_COD = B2_COD AND B2_LOCAL = '00' AND SB2.D_E_L_E_T_ = ''
             INNER JOIN SC6040 SC6 ON B1_COD = C6_PRODUTO AND B2_FILIAL = C6_FILIAL AND C6_LOCAL = B2_LOCAL AND SC6.D_E_L_E_T_ = ''
             INNER JOIN SC5040 SC5 ON C6_NUM = C5_NUM AND C6_FILIAL = C5_FILIAL AND C5_NOTA NOT IN ('XXXXXX','XXXXXXXXX') AND SC5.D_E_L_E_T_ = ''
             WHERE SB1.D_E_L_E_T_='' 
			 AND SUBSTRING(B1_COD, 0, 7) IN ('300302','300308')
             GROUP BY B1_X_SECUN, B1_DESC, B1_CUSTD, B2_QATU, C6_QTDVEN ,C6_QTDENT

             UNION

             SELECT 'FILIAL' AS EMP, B1_COD, B1_DESC, B2_QATU AS ESTOQUE, B1_CUSTD AS CUSTO, SUM(C6_QTDVEN) AS VENDIDO, SUM(C6_QTDENT) AS ENTREGUE, SUBSTRING(B1_COD, 0, 7) AS FAMILIA 
             FROM DBAUX.dbo.SB1010 SBA
             INNER JOIN SB2010 SB2 ON B1_COD = B2_COD AND B2_LOCAL = '01' AND SB2.D_E_L_E_T_ = ''
             INNER JOIN DBAUX.dbo.SC6010 SCC ON B1_COD = C6_PRODUTO AND B2_FILIAL = C6_FILIAL AND C6_LOCAL = B2_LOCAL AND SCC.D_E_L_E_T_ = ''
             INNER JOIN DBAUX.dbo.SC5010 SCD ON C6_NUM = C5_NUM AND C6_FILIAL = C5_FILIAL AND C5_NOTA NOT IN ('XXXXXX','XXXXXXXXX') AND SCD.D_E_L_E_T_ = ''
             WHERE SBA.D_E_L_E_T_=''
			 AND SUBSTRING(B1_COD, 0, 7) IN ('300302','300308')
             GROUP BY B1_COD, B1_DESC, B1_CUSTD, B2_QATU, C6_QTDVEN ,C6_QTDENT
       ) AS TMP
GROUP BY PRODUTO,DESC_PRODUTO
...
as <font color=#088A29 ><script>EscreveHora();</script></font> &nbsp; GERAL ESTOQUE</b></p>
			<table class="jose2"  cellspacing="0">
			<tr>
			<td WIDTH=150  class="" style=" border: 1px solid; border-collapse: collapse;">CODIGO</td>
			<td WIDTH=400  class="" style=" border: 1px solid; border-collapse: collapse;">PRODUTO</td>
...
?>
			...
			<table class="jose2"  cellspacing="0">
			<tr>
			<td style=" border: 1px solid; border-collapse: collapse;" Class="" WIDTH=150 ><?=$linha['PRODUTO']?></td>
			<td style=" border: 1px solid; border-collapse: collapse;" Class="" WIDTH=400 ><?=$linha['DESC_PRODUTO']?></td>
			<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150><?php echo 'R$&nbsp;' . number_format($linha['CUST'], 2, ',', '.');?></td>
			...
			<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150><?php echo '&nbsp;' . number_format($CUSTOQ, 2, ',', '.');?></td>
