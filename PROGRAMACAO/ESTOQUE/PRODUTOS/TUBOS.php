<?php
	...
	$sql = "SELECT PRODUTO,MAX(DESC_PRODUTO) AS DESC_PRODUTO, MAX(CUSTO) AS CUST, SUM(ESTOQUE) AS ESTO, SUM(QTD_VEN) AS VENDIDO, SUM(QTD_ENT) AS ENTREGUE,SUM(CASE EMP WHEN 'MATRIZ' THEN ESTOQUE ELSE 01 END) AS EST_MATRIZ
			...
			FROM (	SELECT 'MATRIZ' AS EMP, B1_COD AS PRODUTO, B1_DESC AS DESC_PRODUTO, B2_QATU AS ESTOQUE, B1_CUSTD AS CUSTO, SUM(C6_QTDVEN) AS QTD_VEN, SUM(C6_QTDENT) AS QTD_ENT, SUBSTRING(B1_COD, 0, 7) AS FAMILIA
					...
					WHERE LEFT(B1_COD,6) IN ('300302') AND SB1.D_E_L_E_T_=''
GROUP BY B1_COD, B1_DESC, B1_CUSTD, B2_QATU
					...
					WHERE LEFT(B1_X_SECUN,6) IN ('300302') AND SB1.D_E_L_E_T_='' 
GROUP BY B1_X_SECUN, B1_DESC, B1_CUSTD, B2_QATU
					...
					WHERE LEFT(B1_COD,6) IN ('300302') AND SBA.D_E_L_E_T_='' 
GROUP BY B1_COD, B1_DESC, B1_CUSTD, B2_QATU
				) AS TMP
			GROUP BY PRODUTO
...
<p align="center" class="jose2">Valor Atualizado em <font color=#088A29><b>
			<script>EscreveData(); </script>
	</font>

	as <font color=#088A29>
		<script>EscreveHora();</script>
	</font> &nbsp; ESTOQUE X PEDIDOS TUBOS</b></p>
<table class="jose2" cellspacing="0" align="center">
	<tr>
...
?>
			...
			<table class="jose2" cellspacing="0" align="center">
				<tr>
					<td style=" border: 1px solid; border-collapse: collapse;" Class="" WIDTH=180><?= $linha['PRODUTO'] ?>
					</td>
					<td style=" border: 1px solid; border-collapse: collapse;" Class="" WIDTH=450>
						<?= $linha['DESC_PRODUTO'] ?></td>
					...
					<!--<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150> - </td>-->
					<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150><a
									href="busca.php?cod=<?= $linha['PRODUTO'] ?>" style="text-decoration: none; color: #04B431;"
									target="_blank"><?php echo '&nbsp;' . number_format($REAL, 2, ',', '.'); ?></a></td>
							<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150><?php
							if ($ESTPED >= 0)
