<?php
	...
	$sql = "SELECT PRODUTO,MAX(DESC_PRODUTO) AS DESC_PRODUTO, MAX(CUSTO) AS CUST, SUM(ESTOQUE) AS ESTO, SUM(QTD_VEN) AS VENDIDO, SUM(QTD_ENT) AS ENTREGUE,
			SUM(CASE EMP WHEN 'MATRIZ' THEN ESTOQUE ELSE 011 END) AS EST_MATRIZ,SUM(CASE EMP WHEN 'FILIAL' THEN ESTOQUE ELSE 0 END) AS EST_FILIAL,
			...
			FROM (	SELECT 'MATRIZ' AS EMP, B1_COD AS PRODUTO, B1_DESC AS DESC_PRODUTO, B2_QATU AS ESTOQUE, B1_CUSTD AS CUSTO, SUM(C6_QTDVEN) AS QTD_VEN, 
					...
					FROM SB1010 SB1
					INNER JOIN SB2010 SB2 ON B1_COD = B2_COD AND B2_LOCAL = '01' AND SB2.D_E_L_E_T_ = '' AND B2_FILIAL = '01IND01'
					LEFT JOIN SC6010 SC6 ON B2_FILIAL = C6_FILIAL AND B1_COD = C6_PRODUTO AND C6_LOCAL = B2_LOCAL AND SC6.D_E_L_E_T_ = '' AND C6_FILIAL = '01IND01'
					LEFT JOIN SC5010 SC5 ON C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND LEFT(C5_NOTA,6) <> 'XXXXXX' AND SC5.D_E_L_E_T_ = '' AND C5_FILIAL = '01IND01' AND C5_XTIPO NOT IN ('P','F')
					...
					INNER JOIN DBAUX.dbo.SB2010 SB2 ON B1_COD = B2_COD AND B2_LOCAL = '01' AND SB2.D_E_L_E_T_ = '' AND B2_FILIAL = '01IND01'
					LEFT JOIN DBAUX.dbo.SC6010 SCC ON B2_FILIAL = C6_FILIAL AND B1_COD = C6_PRODUTO AND C6_LOCAL = B2_LOCAL AND SCC.D_E_L_E_T_ = '' AND B2_FILIAL = '01IND01'
					LEFT JOIN DBAUX.dbo.SC5010 SCD ON C6_FILIAL = C5_FILIAL AND C6_NUM = C5_NUM AND LEFT(C5_NOTA,6) <> 'XXXXXX' AND SCD.D_E_L_E_T_ = '' AND B2_FILIAL = '01IND01' AND C5_XTIPO NOT IN ('P','F')
					WHERE B1_FILIAL='' AND LEFT(B1_COD,6) IN ('300302','300308','300301','800201','800202','300303','800509','300304','300305','300306','300307')
					AND SBA.D_E_L_E_T_='' 
					AND  B1_MSBLQL = '2'
...
as <font color=#088A29 ><script>EscreveHora();</script></font> &nbsp; GERAL ESTOQUE PATMED</b></p>
...
?>			
			...
			<td style=" border: 1px solid; border-collapse: collapse; text-align: right;" class="" WIDTH=150><a href="busca1.php?cod=<?=$linha['PRODUTO']?>&filial=01IND01" style="text-decoration: none; color: #04B431;" target="_blank"><?php 
																												If($linha['ESTO'] >= 0)
